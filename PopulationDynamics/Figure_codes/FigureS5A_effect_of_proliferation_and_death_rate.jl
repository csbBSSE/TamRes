...
function run_sim(List_of_cells,initial_cells,num_replicates,max_time,carrying_capacity,prol_rate,death_rate,mean_dist1,std_dist1,mean_dist2,std_dist2,t_rate_12,t_rate_21,current_score_of_cell,drug_status)
    summary_stats = []
    #div_init = diversity(List_of_cells)
...
no_drug_summary = summary_stats_rep
no_drug_pop = []
for i in no_drug_summary
    push!(no_drug_pop,i[1])
end
no_drug_pop_mean = mean(no_drug_pop)
no_drug_pop_std = 0.434*std(no_drug_pop)./no_drug_pop_mean
print(no_drug_pop_std)
...
with_drug_pop_mean = mean(with_drug_pop)
with_drug_pop_std = 0.434*std(with_drug_pop)./with_drug_pop_mean
with_drug_pop_mean

#%%
#WITH LOW DRUG CASE
drug_status = 1
mean_dist1, std_dist1, mean_dist2, std_dist2, t_rate_12, t_rate_21, prol_rate, death_rate, current_score_of_cell = (-2, 0, 2, 0, 0, 0, 0.99, 0.01, pop_initial)
lock = Threads.SpinLock()
summary_stats_rep = []
Threads.@threads for rep in 1:num_replicates
    List_of_cells = init_double_gaussian(mean_dist1,std_dist1,mean_dist2,std_dist2,t_rate_12,t_rate_21,initial_cells,prol_rate,death_rate,current_score_of_cell)
...
with_drug_less_summary = summary_stats_rep
with_drug_less_pop = []
for i in with_drug_less_summary
    push!(with_drug_less_pop,i[1])
end
with_drug_less_pop_mean = mean(with_drug_less_pop)
with_drug_less_pop_std = 0.434*std(with_drug_less_pop)./with_drug_less_pop_mean
with_drug_less_pop_mean

#%%
#WITH High DRUG CASE
drug_status = 1
mean_dist1, std_dist1, mean_dist2, std_dist2, t_rate_12, t_rate_21, prol_rate, death_rate, current_score_of_cell = (-2, 0, 2, 0, 0, 0, 0.9, 0.5, pop_initial)
lock = Threads.SpinLock()
summary_stats_rep = []
Threads.@threads for rep in 1:num_replicates
    List_of_cells = init_double_gaussian(mean_dist1,std_dist1,mean_dist2,std_dist2,t_rate_12,t_rate_21,initial_cells,prol_rate,death_rate,current_score_of_cell)
...
with_drug_high_summary = summary_stats_rep
with_drug_high_pop = []
for i in with_drug_high_summary
    push!(with_drug_high_pop,i[1])
end
with_drug_high_pop_mean = mean(with_drug_high_pop)
with_drug_high_pop_std = 0.434*std(with_drug_high_pop)./with_drug_high_pop_mean

#%%
plot(1:max_time,no_drug_pop_mean.+1,ribbon=no_drug_pop_std, yaxis=:log,fmt=:png,dpi =800,legend = false, grid = false)#,markerstrokewidth=0.2, linewidth=2, thickness_scaling = 1)
plot!(1:max_time,with_drug_high_pop_mean.+1,ribbon=with_drug_high_pop_std, yaxis=:log,fmt=:png,dpi = 800,legend = false, grid = false)#,markerstrokewidth=0.2, linewidth=2, thickness_scaling = 1)
plot!(1:max_time,with_drug_pop_mean.+1,ribbon=with_drug_pop_std, yaxis=:log,fmt=:png,dpi = 800,legend = false, grid = false)#,markerstrokewidth=0.2, linewidth=2, thickness_scaling = 1)
plot!(1:max_time,with_drug_less_pop_mean.+1,ribbon=with_drug_less_pop_std, yaxis=:log,fmt=:png,dpi = 800,legend = false, grid = false)#,markerstrokewidth=0.2, linewidth=2, thickness_scaling = 1)
ylims!((0.8,10^5))
xlims!((0,35))
savefig("FigureS5A.png")
